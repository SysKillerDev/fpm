...
class FPM::Command < Clamp::Command
  ...
  def execute
    ...
    # Flags in packages defined as "--bar-bar" become named "--<packagetype>-bar-bar"
    # They are stored in 'settings' as :gem_bar_bar.
    input.attributes ||= {}

    # Iterate over all the options and set their values in the package's
    # attribute hash.
    #
    # Things like '--bar-bar' will be available as pkg.attributes[:bar_bar]
    self.class.declared_options.each do |option|
      with(option.attribute_name) do |attr|
        next if attr == "help"
        # clamp makes option attributes available as accessor methods
        # --bar-bar is available as 'bar_bar'. Put these in the package
        # attributes hash. (See FPM::Package#attributes)
        # 
        # In the case of 'flag' options, the accessor is actually 'bar_bar?'
        # instead of just 'bar_bar'
    ...
    # means package name or paths to the packages (rails, bar-1.0.gem, django,
    # bar/setup.py, etc)
    args.each do |arg| 
      input.input(arg) 
    end
