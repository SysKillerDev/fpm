...
class FPM::Package::RPM < FPM::Package
  ...
  option "--verbatim-gem-dependencies", :flag,
           ...
           "dependency names. This flag will use the old 'rubygem-bar' " \
           "names in rpm requires instead of the redhat style " \
           "rubygem(bar).", :default => false
  ...
  def converted_from(origin)
    if origin == FPM::Package::Gem
      ...
      # Convert 'rubygem-bar' provides values to 'rubygem(bar)'
    ...
    # if --ignore-iteration-in-dependencies is true convert bar = X, to
    # bar >= X , bar < X+1
