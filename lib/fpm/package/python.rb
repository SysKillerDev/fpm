...
class FPM::Package::Python < FPM::Package
  # Flags '--bar' will be accessable  as attributes[:python_bar]
  ...
  def load_package_info(setup_py)
    ...
    # name prefixing is optional, if enabled, a name 'bar' will become
    # 'python-bar' (depending on what the python_package_name_prefix is)
    ...
    # convert python-Foo to python-bar if flag is set
    ...
    if !attributes[:no_auto_depends?] and attributes[:python_dependencies?]
      self.dependencies = metadata["dependencies"].collect do |dep|
        ...
        # dependency name prefixing is optional, if enabled, a name 'bar' will
        # become 'python-bar' (depending on what the python_package_name_prefix
        # is)
        name = fix_name(name) if attributes[:python_fix_dependencies?]

        # convert dependencies from python-Foo to python-bar
  ...
  # Some PyPI packages can be named 'python-bar', so we don't want to end up
  # with a package named 'python-python-bar'.
  # But we want packages named like 'pythonweb' to be suffixed
  # 'python-pythonweb'.
  def fix_name(name)
    if name.start_with?("python")
      # If the python package is called "python-bar" strip the "python-" part while
