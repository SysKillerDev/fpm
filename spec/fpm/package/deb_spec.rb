...
describe FPM::Package::Deb do
  ...
  describe "#output" do
    before :all do
      ...
      @original.attributes[:deb_field] = { "bar" => "bar" }
    ...
    context "when read with dpkg", :if => have_dpkg_deb do
      ...
      it "should have a custom field 'bar: bar'" do
        insist { dpkg_field("bar") } == "bar"
      end

      it "should have the correct Conflicts" do
        insist { dpkg_field("Conflicts") } == "bar (<< 123)"
      end
